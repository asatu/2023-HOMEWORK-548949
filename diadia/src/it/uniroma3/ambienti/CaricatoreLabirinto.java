...
public class CaricatoreLabirinto {
	...
	private static final String MAGICHE_MARKER = "Stanze Magiche:";

	private static final String CHIUSE_MARKER = "Stanze Chiuse:";

	private static final String BUIE_MARKER = "Stanze Buie:";
	 ...
	 *  Esempio di un possibile file di specifica di un labirinto (vedi POO-26-eccezioni-file.pdf)
		...
		Personaggi: Mago biblioteca, Strega N10
	...
	private void leggiECreaPersonaggi() throws FormatoFileNonValidoException {
		...
		for(String specificaPersonaggio : separaStringheAlleVirgole(specifichePersonaggi)) {
			...
			allocaPersonaggio(nomePersonaggio, nomeAttrezzo, pesoAttrezzo, nomeStanza);
		}

	}

	private void allocaPersonaggio(String nomePersonaggio, String nomeAttrezzo, String pesoAttrezzo, String nomeStanza) throws FormatoFileNonValidoException {
		try {
			String descrizione;
			int peso;
			...
			if(nomePersonaggio.equals("Mago")) {
				descrizione = "Ciao, Sono un mago!";
				Mago mago = new Mago(nomePersonaggio, descrizione, attrezzo);
				this.nome2stanza.get(nomeStanza).setPersonaggio(mago);
			}
			if(nomePersonaggio.equals("Strega")) {
				descrizione = "Ciao, Sono una strega!" ;
				Strega strega = new Strega(nomePersonaggio, descrizione);
				this.nome2stanza.get(nomeStanza).setPersonaggio(strega);
			}
			if(nomePersonaggio.equals("Cane")) {
				descrizione = "Ciao, Sono un cane!" ;
				Cane cane = new Cane(nomePersonaggio, descrizione, "cibo preferito", attrezzo);
	...
	private void leggiEImpostaUscite() throws FormatoFileNonValidoException {
		...
		try (Scanner scannerDiLinea = new Scanner(specificheUscite)) {			
			...
			while (scannerDiLinea.hasNext()) {
				...
				String dirs = scannerDiLinea.next();
				Direzione dir = null;
				dir.valueOf(dirs);
